...
export class RegexAnyTagMatcher extends Entity implements IPickleMatcher {
    ...
    constructor(tagPatterns: string[], regexOptions?: IRegexOptions) {
        ...
        this.match = (pickle: Pickle) => regexes.some(regex => pickle.tags?.some(t => regex.test(t.name)));
    }
    ...
    public getPattern(): string {
        ...
        return `anyTag: [${this.tagPatterns.join(', ')}]${flags ? ` (flags: ${flags})` : ''}`;
    }
