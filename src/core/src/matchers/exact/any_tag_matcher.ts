...
export class ExactAnyTagMatcher extends Entity implements IPickleMatcher {
    ...
    constructor(tags: string[], ignoreCase: boolean = false) {
        super('exact-any-tag');
        this.level = MatcherLevel.ANY_TAG;
        ...
        if (ignoreCase) {
            ...
            this.match = (pickle: Pickle) => 
                lowerTags.some(tag => pickle.tags?.some(t => t.name.toLowerCase() === tag));
        } else {
            this.match = (pickle: Pickle) => 
                tags.some(tag => pickle.tags?.some(t => t.name === tag));
        }
    }

    public getPattern(): string {
        const ignoreCaseStr = this.ignoreCase ? ' (ignoreCase)' : '';
        return `anyTag: [${this.tags.join(', ')}]${ignoreCaseStr}`;
    }
